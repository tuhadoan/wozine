...
class DTBreadcrumb {
	...
	public function output($args = array()){
		...
		$defaults = array(
				'home' => esc_html_e( 'Home', 'dawnthemes' ),
				'delimiter' => '',
				'front_page' => false,
				'show_blog' => true,
		...
		/* If viewing any type of archive. */
			...
			if ( is_category() || is_tag() || is_tax() ) {
				...
				if(is_numeric(get_query_var('w') && 0 !== get_query_var('w') ))
					$item['last'] =  sprintf( esc_html_e('Weekly Archive for: "%s"','dawnthemes'),get_the_time('W'));
				elseif ( is_day() )
				$item['last'] = sprintf( esc_html_e('Daily Archive for: "%s"','dawnthemes'),get_the_time('F jS, Y'));
				elseif ( is_month() )
				$item['last'] =  sprintf( esc_html_e('Monthly Archive for: "%s"','dawnthemes'),get_the_time('F jS, Y'));
				elseif ( is_year() )
				$item['last'] =  sprintf(esc_html_e('Yearly Archive for: "%s"','dawnthemes'),get_the_time('Y'));
			}
		
			else if ( is_author() )
				$item['last'] =  sprintf(esc_html_e('Author Archive for: "%s"','dawnthemes'),get_the_author_meta( 'display_name', $wp_query->post->post_author ));
		}
		
		/* If viewing search results. */
		else if ( is_search() )
			$item['last'] =  esc_html_e('Search','dawnthemes');//sprintf(esc_html_e('search Results for: "%s"','dawnthemes'),stripslashes( strip_tags( get_search_query() ) ));
			
		/* If viewing a 404 error page. */
		else if ( is_404() )
			$item['last'] = esc_html_e( 'Page Not Found', 'dawnthemes' );
	...
	protected function _bbpress_items() {
		...
		elseif ( bbp_is_single_topic() ) {
			...
			if ( bbp_is_topic_split() )
				$item[] = esc_html_e( 'Split', 'dawnthemes' );
	
			elseif ( bbp_is_topic_merge() )
			$item[] = esc_html_e( 'Merge', 'dawnthemes' );
	
			elseif ( bbp_is_topic_edit() )
			$item[] = esc_html_e( 'Edit', 'dawnthemes' );
		}
