...
if ( ! class_exists( 'DT_Mega_Walker' ) ) :
	...
	class DT_Mega_Walker extends DT_Walker {
		...
		public function end_el( &$output, $item, $depth = 0, $args = array() ) {
			$output .= "</li>\n";
			$header_style = dt_get_theme_option('header-style','below');
			if($header_style =='center2'){
				static $static_nav_count = 0;
				if($depth === 0){
					$logo_url = dt_get_theme_option('logo');
					$logo_fixed_url = dt_get_theme_option('logo-fixed','');
					$logo_transparent_url = dt_get_theme_option('logo-transparent','');
					$logo_mobile_url = dt_get_theme_option('logo-mobile','');
					if(empty($logo_fixed_url))
						$logo_fixed_url = $logo_url;
					if(empty($logo_mobile_url))
						$logo_mobile_url = $logo_url;
					$menu_transparent = dt_get_theme_option('menu-transparent',0);
					if($menu_transparent)
